...
DEFAULT_HOST = "https://xhappcmszbc.zbc4k.app/api/v1"

# 备用域名列表 (目前只有主域名, 后续添加备用域名直接在此扩展)
_BACKUP_HOSTS = [
    "https://xhappcmszbc.zbc4k.app/api/v1",
]
...
class Spider(_BaseSpider):
    ...
    def _probe_hosts(self, hosts):
        """并发探测多个域名, 返回最快响应的域名
        探测方式: GET /init (免签接口), 检查返回 data.types
        超时: 5 秒
        """
        ...
        def _try(host):
            try:
                url = host.rstrip("/") + "/init?platform=app"
                start = time.time()
                req = urllib.request.Request(url, headers=self._headers())
                r = urllib.request.urlopen(req, timeout=5, context=_ssl_ctx)
                resp = json.loads(r.read())
                elapsed = time.time() - start
                dec = self._decrypt(resp)
                if dec and isinstance(dec.get("data"), dict) and dec["data"].get("types"):
                    print("[真不错4K] 域名探测成功: %s (%.2fs)" % (host, elapsed))
                    return host
            except Exception:
                pass
            return None
                ...
                if result:
                    for f2 in futures:
                        if not f2.done():
                            f2.cancel()
                    return result
        print("[真不错4K] 所有域名探测失败, 使用默认: %s" % hosts[0])
        return hosts[0]

    def _ensure_host(self):
        """确保 HOST 可用, 带 10 分钟冷却缓存"""
        now = time.time()
        if self._effective_host and (now - self._last_host_check) < _HOST_CACHE_TTL:
        ...
        candidates = []
        if self._user_host and self._user_host not in candidates:
            candidates.append(self._user_host)
        ...
        if len(candidates) == 1:
            # 单域名直接用, 不做探测
            self._effective_host = candidates[0]
            print("[真不错4K] 单域名模式: %s" % candidates[0])
        else:
            # 多域名并发探测选最快
            print("[真不错4K] 并发探测 %d 个域名..." % len(candidates))
            best = self._probe_hosts(candidates)
            self._effective_host = best
    ...
    def _request(self, path, params=None, method="GET", body=None, retries=2):
        ...
        vbox 适配: urlopen 传入 context=_ssl_ctx
        """
        self._ensure_host()
        for attempt in range(retries + 1):
            ...
            try:
                ...
                if attempt < retries:
                    print("[真不错4K] 请求异常(重试 %d/%d): %s, path=%s" % (attempt + 1, retries, e, path))
                else:
                    print("[真不错4K] 请求异常(已放弃): %s, path=%s" % (e, path))
            time.sleep(0.6)
        return None
    ...
    def init(self, extend=""):
        ...
        if extend:
            ...
            if extend.startswith("http"):
                # 纯 URL: 作为自定义 API 地址
                self._user_host = extend.rstrip("/")
                if not self._user_host.endswith("/api/v1"):
                    self._user_host = self._user_host.rstrip("/") + "/api/v1"
            else:
                try:
                    ext_data = json.loads(extend)
                    if isinstance(ext_data, dict) and ext_data.get("base_url"):
                        self._user_host = ext_data["base_url"].rstrip("/")
                        if not self._user_host.endswith("/api/v1"):
                            self._user_host = self._user_host.rstrip("/") + "/api/v1"
                except Exception:
                    pass
        ...
        # 域名探测
        self._ensure_host()
        print("[真不错4K] init 完成, host=%s" % HOST)
        return ""
    ...
    def _load_live(self):
        ...
        print("[真不错4K] 直播加载: %d 分类, %d 频道" % (len(cats), len(self._live_channels)))
    ...
    def _load_types(self):
        ...
        if not types:
            ...
            try:
                ...
                resp = json.loads(urllib.request.urlopen(req, timeout=15, context=_ssl_ctx).read())
                dec = self._decrypt(resp)
    ...
    def homeContent(self, filter):
        ...
        try:
            ...
            pass
        print("[真不错4K] homeContent: %d 分类" % len(classes))
    ...
    def categoryContent(self, tid, pg, filter1, ext):
        ...
        if not videos:
            print("[真不错4K] 分类 tid=%s page=%d: 0 条" % (tid, pg))
            return {"list": [], "limit": PAGE_SIZE, "total": pg}
        print("[真不错4K] 分类 tid=%s page=%d: %d 条" % (tid, pg, len(videos)))
    ...
    def _live_category(self, pg, ext):
        ...
        print("[真不错4K] 直播分类 page=%d group=%s: %d 条" % (pg, group, len(videos)))
    ...
    def detailContent(self, ids):
        ...
        print("[真不错4K] 详情: %s, %d 条线路" % (info.get("vod_name", ""), len(sources)))
        return {"list": [vod]}
    ...
    def searchContent(self, key, quick, pg="1"):
        ...
        if d and isinstance(d.get("data"), dict):
            ...
            try:
                ...
                total = pg + (1 if len(videos) >= PAGE_SIZE else 0)
        print("[真不错4K] 搜索 '%s': %d 条" % (key, len(videos)))
