...
class Spider(Spider):
    ...
    def homeContent(self, filter):
        ...
        try:
            ...
            print(f'ThreadPoolExecutor unavailable, fallback to serial: {e}')
            for func in [self.process_bili, self.process_douyin, self.process_douyu]:
                platform, filter_data = func()
                if filter_data:
                    filters[platform] = filter_data
    ...
    def wyccDetail(self, ids):
        try:
            vdata = self.getpq_text(f'{self.hosts[ids[0]]}/{ids[1]}', self.headers[0])
    ...
    def douyuplay(self, ids):
        try:
            ...
            try:
                ...
                print(f'ThreadPoolExecutor unavailable, fallback to serial: {e}')
                for quality in ldata:
                    self.douyufp(sdata, quality, headers, self.hosts[ids[0]], result_obj)
