...
import json,random,string,time,hashlib
...
class Spider(Spider):
    ...
    def _api(self,path):
        ...
        try:
            r=self.fetch(url,headers=self._headers(),timeout=12,verify=False)
            j=r.json()
        ...
        for h in self.hs:
            for _ in range(3):
                ...
                try:
                    ...
                    r=self.post(domain+'/api/user/traveler',data=json.dumps(body),headers={**hd,'Content-Type':'application/json'},timeout=8,verify=False)
                    d=json.loads(r.text).get('data',{})
    ...
    def _img_asset(self,u):
        ...
        try:
            r=self.fetch(self._img_url(u),headers={'User-Agent':'Dalvik/2.1.0 (Linux; U; Android 11; M2012K10C Build/RP1A.200720.011)'},timeout=15,verify=False)
