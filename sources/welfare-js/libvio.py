...
class Spider(_BaseSpider):
    ...
    def localProxy(self, params):
        """代理 m3u8/TS 分片，附加 UA + Referer 绕过防盗链"""
        try:
            import base64 as _b64
            # params 格式: base64 编码的 JSON {"url": "..."} 或原始 dict
            if isinstance(params, str):
                try:
                    params = json.loads(_b64.b64decode(params))
                except Exception:
                    params = json.loads(params) if params else {}
            target_url = str(params.get("url") or params.get("path") or "").strip()
            if not target_url:
