...
"""
VV视频 — vbox 适配版
站点: 351.dofd1fe.com:25118 (动态 API 发现)

vbox 适配：
1. 基类导入加 try/except + as _B（避免类名冲突）
2. Crypto.Protocol.KDF.PBKDF2 → hashlib.pbkdf2_hmac（IPA 中 Crypto.Protocol 不存在）
3. Crypto.Hash.SHA256 → 删除（hashlib 内置）
4. AES.MODE_GCM decrypt_and_verify → AES-ECB 实现 AES-CTR 解密（IPA 中 GCM 不可用）
5. AES.MODE_CBC 保留（IPA 中可用，用于 localProxy 封面解密）
6. getDependence 声明依赖
7. 添加 warnings 抑制
"""
import base64
import html
...
# Crypto 包精简适配：IPA 中仅 CBC/ECB 可用，GCM/KDF/Hash 不可用
try:
...
class Spider(_B):
    ...
    def homeContent(self, filter=False):
        ...
        try:
            ...
            print('[VV] 分类加载失败，使用内置分类: %s' % error)
        if not classes:
            classes = [
    ...
    # ============================================================
    # 信封解密（核心替换：PBKDF2→hashlib, AES-GCM→AES-ECB 实现 CTR）
    # ============================================================
    ...
    def _decrypt_envelope(self, encrypted, password):
        try:
            ...
            # ============================================================
            # 替换 PBKDF2(SHA256) → hashlib.pbkdf2_hmac（标准库，IPA 可用）
            # ============================================================
            ...
            # ============================================================
            # 替换 AES-GCM decrypt_and_verify → AES-ECB 实现 CTR 解密
            # GCM 解密 = AES-CTR（跳过 tag 验证即可获得明文）
            # IPA 中 MODE_ECB 可用，MODE_GCM 不可用
            # ============================================================
            plain = self._aes_ctr_decrypt(key, iv, ciphertext[:-16])
        except Exception:
            # 回退：XOR 解密（不依赖 Crypto）
            raw = self._b64decode(encrypted)
            key_bytes = password.encode('utf-8')
            if not key_bytes:
                raise ValueError('解密 key 为空')
            plain = bytes(value ^ key_bytes[index % len(key_bytes)] for index, value in enumerate(raw))
    ...
    @staticmethod
    def _aes_ctr_decrypt(key, iv, ciphertext):
        """
        AES-CTR 解密（使用 AES-ECB 实现）
        GCM 使用 12 字节 nonce，计数器从 nonce||0x00000001 开始递增
        """
        cipher = AES.new(key, AES.MODE_ECB)
        # GCM counter: 12-byte nonce + 4-byte counter (starts at 1)
        counter_block = iv + b'\x00\x00\x00\x01'
        plain = b''
        offset = 0
        while offset < len(ciphertext):
            keystream = cipher.encrypt(counter_block)
            block = ciphertext[offset:offset + 16]
            plain += bytes(a ^ b for a, b in zip(block, keystream[:len(block)]))
            # 递增计数器（128-bit big-endian）
            counter_int = int.from_bytes(counter_block, 'big') + 1
