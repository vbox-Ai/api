...
class Spider(BaseSpider):
    default_host = "https://bkpk82.baokuanpk.cc"
    ...
    def _resp_json(self, r):
        if isinstance(r, str):
            return json.loads(r)
        if hasattr(r, 'json'):
            return r.json()
        return json.loads(self._resp_text(r))

    def init(self, extend):
        self.host = getattr(self, 'host', '') or self.default_host
        self.api_url = self.host + "/api.php/provide/vod/"
        self.use_api = False
    ...
    def _api_home_video(self):
        videos = []
        try:
            res = self.fetch(self.api_url + "?ac=list&pg=1", headers=headerx, timeout=10, verify=False)
            data = self._resp_json(res)
            if data.get('code') == 1:
    ...
    def _html_home_video(self):
        videos = []
        try:
            res = self.fetch(self.host + '/bb/', headers=headerx, timeout=10, verify=False)
    ...
    def _api_category_content(self, cid, pg, filter, ext):
        ...
        try:
            ...
            if m:
                tid = m.group(1)
            url = self.api_url + f"?ac=list&t={tid}&pg={pg}"
            res = self.fetch(url, headers=headerx, timeout=10, verify=False)
            data = self._resp_json(res)
            if data.get('code') == 1:
    ...
    def _html_category_content(self, cid, pg, filter, ext):
        ...
        try:
            res = self.fetch(url=url, headers=headerx, timeout=10, verify=False)
            html = self._resp_text(res)
            if len(html) >= 500:
                videos = self._extract_videos_from_html(html)
    ...
    def _api_detail_content(self, did):
        ...
        result = {}
        try:
            url = self.api_url + f"?ac=detail&ids={did}"
            res = self.fetch(url, headers=headerx, timeout=10, verify=False)
            data = self._resp_json(res)
            if data.get('code') == 1 and data.get('list'):
                item = data['list'][0]
    ...
    def _html_detail_content(self, did):
        ...
        try:
            if did.isdigit():
                ...
                did = '/' + did
            detail_url = self.host + did if not did.startswith('http') else did
            print(f"[_html_detail] 请求详情页: {detail_url}")

            res = self.fetch(url=detail_url, headers=headerx, timeout=10, verify=False)
            html = self._resp_text(res)
            if len(html) < 500:
                return result
    ...
    def _get_m3u8_from_play_page(self, play_page_path):
        ...
        """
        try:
            play_url_full = self.host + play_page_path if not play_page_path.startswith('http') else play_page_path
            print(f"[_get_m3u8] 请求播放页: {play_url_full}")

            res = self.fetch(play_url_full, headers=headerx, timeout=10, verify=False)
            html = self._resp_text(res)
            if len(html) < 500:
                return "", "naixx"
    ...
    def _api_search(self, key, quick, page):
        ...
        videos = []
        try:
            url = self.api_url + f"?ac=list&wd={quote(key)}&pg={page}"
            res = self.fetch(url, headers=headerx, timeout=10, verify=False)
            data = self._resp_json(res)
            if data.get('code') == 1:
                for item in data.get('list', []):
    ...
    def _html_search(self, key, quick, page):
        ...
        videos = []
        try:
            search_url = self.host + f'/bb/index.php/vod/search.html?wd={quote(key)}&page={page}'
            res = self.fetch(search_url, headers=headerx, timeout=10, verify=False)
            html = self._resp_text(res)
            if len(html) > 500:
                videos = self._extract_videos_from_html(html)
    ...
    def localProxy(self, params):
        ...
        return [404, "text/plain", b"unsupported type"]

    def _proxy_m3u8(self, params):
        url = params.get('url', '')
        referer = params.get('referer', self.host)
        if not url:
            return [404, "text/plain", b"no url"]
        text = self._get_m3u8_content(url, referer)
        if not text:
            return [404, "text/plain", b"m3u8 download failed"]
        # 广告清洗 + 相对路径转绝对
    ...
    def _get_m3u8_content(self, url, referer):
        try:
            ...
            resp = self.fetch(url, headers=headers, timeout=10, verify=False)
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        # 使用代理链接（代理内部会进行广告清洗）
        proxy_url = self._proxy_m3u8_url(m3u8_url, self.host + '/')
        media_header = {
            "User-Agent": headerx['User-Agent'],
            "Referer": self.host + '/',
            "Origin": self.host
