# -*- coding: utf-8 -*-
"""
小姐姐聚合 TVBox Spider — vbox 适配版
54个随机视频API聚合，ThreadPoolExecutor 并发获取

vbox 适配：
1. 基类导入 try/except + as _B（修复类名冲突）
2. 补 getDependence(['requests'])
3. header 格式 → dict
4. homeContent 补 filters
5. homeVideoContent 改为返回 {"list": items}
6. playerContent 参数名改为标准 (flag, id, vipFlags)
7. timeout 3s → 8s
8. 补 destroy + localProxy
"""
import base64, json, re, random, warnings
...
class Spider(_B):
    ...
    def isVideoFormat(self, url):
        return any(ext in url.lower() for ext in ['.mp4', '.m3u8', '.flv', '.ts', '.mov', '.webm'])
    ...
    def playerContent(self, flag, id, vipFlags=None):
        ...
        # id 可能是 vod_play_url 格式 "播放$http://..."
        if id and "$" in id:
            url = id.split("$", 1)[-1]
