# -*- coding: utf-8 -*-
"""
四色 - 图片版（自包含，无需依赖 sise.py）
暴露图片区 + 撸撸区两大分类，用于福利漫画栏目。
封面图和详情图均返回原始 URL（不走本地代理），由客户端 PlatformImageLoader 通过 Referer 防盗链加载。

iOS CPython 环境中脚本是独立下载的，不能 from sise import，因此本文件内联了所有必要代码。
"""
...
class Spider(_B):
    ...
    def detailContent(self, ids):
        ...
        return {'list': [{'vod_id': play_url, 'vod_name': title, 'vod_pic': img,
            ...
            'vod_actor': '', 'vod_director': '', 'vod_content': '',
            'vod_play_from': '圖片集', 'vod_play_url': '圖片集$' + play_url}]}
