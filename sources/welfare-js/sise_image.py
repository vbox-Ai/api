...
- 仅保留图片区和撸撸区的图片分类
- 输出格式适配漫画浏览（pics:// 协议）
- 基于原版 sise.py 精简，移除视频/小说/有声相关逻辑
"""
...
class Spider(_B):
    def init(self, ext=''):
        ...
        print(f'[sise_image]_discover_domain返回: {self.H}')
        test_url = self.H + '/index/home.html'
        print(f'[sise_image]测试首页: {test_url}')
        try:
            r = self.s.get(test_url, timeout=10, allow_redirects=True)
            print(f'[sise_image]首页响应: {r.url} 状态:{r.status_code} 长度:{len(r.text)} 含enter:{"enter-content" in r.text}')
            if 'enter-content' in r.text and r.url != test_url:
                parsed = urlparse(r.url)
                self.H = f'{parsed.scheme}://{parsed.netloc}'
                print(f'[sise_image]重定向修正域名: {self.H}')
        except Exception as e:
            print(f'[sise_image]首页测试异常: {e}')
        print(f'[sise_image]最终域名: {self.H}')
        try:
    ...
    def _get(self, u):
        if not u.startswith('http'):
            u = self.H + u
        try:
            print(f'[sise_image]请求: {u}')
            r = self.s.get(u, timeout=20, headers={'User-Agent': U, 'Referer': self.H + '/'}, allow_redirects=True)
            r.encoding = 'utf-8'
            print(f'[sise_image]响应: {r.url} 状态:{r.status_code} 长度:{len(r.text)} 含enter:{("enter-content" in r.text)}')
            if r.url != u:
                print(f'[sise_image]重定向: {u} -> {r.url}')
            return r.text
        except Exception as e:
            print(f'[sise_image]请求异常: {e}')
            return ''
