...
def extract_play_url(html_text, host, fetch_func=None):
    """提取播放地址，支持传入自定义 fetch 函数（vbox适配）"""
    import requests as _requests
    _fetch = fetch_func or (lambda url, headers=None: _requests.get(url, headers=headers, timeout=10, verify=False))
    if not html_text:
        return ""
    patterns = [
        r'(https?://[^\s"\']+\.m3u8[^\s"\']*)',
    ...
    for pat in patterns:
        ...
        if m:
            ...
            if pat.endswith('})'):
                ...
                try:
                    r = _fetch(iframe_url, headers={
                        "User-Agent": "Mozilla/5.0",
                        "Referer": host
                    })
                    return extract_play_url(r.text, iframe_url, fetch_func=fetch_func)
                except:
                    pass
...
class Spider(BaseSpider):
    ...
    def init(self, extend=""):
        # vbox 适配: 域名注入（_vbox_effective_hosts 优先于 extend）
        try:
            _hosts = globals().get('_vbox_effective_hosts', [])
            if _hosts:
                self.host = str(_hosts[0]).rstrip('/')
                self.headers["Referer"] = self.host + "/"
            elif extend and extend.startswith("http"):
    ...
    def _post_fetch(self, url, data, referer=None):
        ...
        if referer:
            h["Referer"] = referer
        try:
            r = self.fetch(url, headers=h, data=data)
            return r.text if hasattr(r, 'text') else (r if isinstance(r, str) else "")
        except Exception as e:
    ...
    def detailContent(self, ids):
        try:
            ...
            play_url_raw = extract_play_url(html_text, self.host, fetch_func=self._fetch)
            if not play_url_raw:
    ...
    def playerContent(self, flag, id, vipFlags=None):
        try:
            ...
            if id.startswith("http"):
                ...
                if html_text:
                    play_url = extract_play_url(html_text, self.host, fetch_func=self._fetch)
                    if play_url:
                        result["url"] = play_url
                        result["header"] = {
                            "Referer": id,
