...
# -*- coding: utf-8 -*-
"""
福利专区专用 Spider 脚本：艾旦影视

文件位置：vbox-Ai/api/sources/welfare-js/aidan.py

说明：
- 本脚本仅作为福利专区远程源配置（welfare_platforms.json）的标记和安卓端参考使用
- iOS 端 vbox 不执行此脚本，而是由原生 Swift Service 重新实现相同逻辑
- 必须配置 visibleInNormalSpider=false, visibleInGlobalSearch=false, visibleInHome=false
  以确保该源只出现在福利专区，不参与普通首页、全局搜索和普通播放链路

数据源：CMS V10 API (https://www.lovedan.net/api.php/provide/vod/)
内容：同时包含视频类（有 vod_play_url）和图片/套图类（无 vod_play_url）
"""
...
class Spider(Spider):
    ...
    def init(self, extend=""):
        ...
        # 保留原始分类顺序，block_ids / block_names 过滤掉非福利内容
        self.class_order = [
    ...
    def _pic(self, item):
        p = (item.get("vod_pic")
             or item.get("vod_pic_thumb")
             or item.get("vod_pic_slide")
             or item.get("vod_pic_screenshot")
             or "")
        return self._fix(p)
    ...
    def _ok(self, item):
        ...
        return (cid not in self.block_ids
                and tname not in self.block_names
                and vclass not in self.block_names)

    def _classes(self, classes):
        mp = {str(i.get("type_id", "")): i.get("type_name", "") for i in classes}
        arr = [{"type_id": i, "type_name": mp[i]}
               for i in self.class_order
               if i in mp and i not in self.block_ids]
        used = set([i["type_id"] for i in arr])
        arr += [{"type_id": str(i.get("type_id", "")),
                 "type_name": i.get("type_name", "")}
                for i in classes
                if str(i.get("type_id", "")) not in used
                and str(i.get("type_id", "")) not in self.block_ids
                and i.get("type_name", "") not in self.block_names]
        return arr
