...
class Spider(BaseSpider):
    ...
    def __init__(self):
        super().__init__()
        # vbox 注入的 self.host 优先；未注入时使用默认站点
        ...
        # 会话缓存（内存字典）
        self._session_cache = {}
        self._session_cache_ttl = 1800  # 30 分钟
    ...
    def _save_cached_site(self, host):
        self._speed_cache = {'host': host, 'ts': time.time()}
    ...
    def _resolve_host(self, portal):
        ...
        """
        try:
            r = self.fetch(portal, headers=self.headers, timeout=TIMEOUT,
    ...
    def _select_best_site(self):
        ...
        # 优先使用 vbox 注入的 effective host
        injected_host = getattr(self, 'host', None)
        if injected_host and injected_host not in ('', SITES[0]['host']):
            resolved = self._resolve_host(injected_host)
            if resolved:
                self.host = resolved
    ...
    def localProxy(self, param):
        try:
            # vbox 可能传入 dict 或 query string
            if isinstance(param, str):
                parsed = parse_qs(param)
