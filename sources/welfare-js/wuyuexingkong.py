...
"""
五月星空 — vbox 适配版
站点: d8v8rjm9.bbmm277.sbs

vbox 适配：
1. 基类导入加 try/except + as _B
2. fetch_html 简化为 self.fetch().text（基类返回 Response 对象）
3. 添加 warnings 抑制 + getDependence
4. localProxy 返回 pass
"""
import re
import json
import warnings
...
class Spider(_B):
    ...
    def fetch_html(self, url):
        """简化：基类 fetch 返回 Response 对象，直接取 .text"""
        try:
            return self.fetch(url, headers=self.header, timeout=15).text
        except Exception:
            return ""
