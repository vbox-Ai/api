...
# 2. import requests 移到顶部 + try/except
# 3. 添加 warnings 抑制 + getDependence(['lxml', 'requests'])
# 4. lxml.etree 已在 IPA 中集成，直接使用
# 5. homeContent 补 filters:{}
...
class Spider(_B):
    ...
    def _parseList(self, tree):
        ...
        if not lis:
            ...
            for a in lis:
                ...
                if title:
                    videos.append({"vod_id": vid, "vod_name": title, "vod_pic": poster, "vod_remarks": ""})
            return videos
        for li in lis:
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        return {"parse": 0, "url": final_url, "header": header, "jx": 0}
