...
class Spider(BaseSpider):
    ...
    def init(self, extend=""):
        self.host = getattr(self, 'host', '') or self.__class__.host

    def _get(self, url):
        try:
            r = self.fetch(url, headers=self.headers, timeout=20, verify=False)
            if isinstance(r, str):
                return r
    ...
    def localProxy(self, param):
        try:
            ...
            url = unquote(url)
            r = self.fetch(url, headers={'User-Agent': self.headers['User-Agent'], 'Referer': self.host + '/'}, timeout=15, verify=False)
            data = r.content if hasattr(r, 'content') else (r.encode('utf-8') if isinstance(r, str) else b'')
            if not data:
                return [404, 'text/plain', b'not found']
