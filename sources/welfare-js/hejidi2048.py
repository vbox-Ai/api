...
import time
import random
from urllib.parse import quote, urljoin, unquote, urlparse, urlencode
...
class Spider(BaseSpider):
    ...
    def _fetch(self, url, referer=None, retries=3):
        for attempt in range(retries):
            try:
                ...
                r = self.fetch(url, headers=self._get_headers(referer), timeout=20, verify=False)
    ...
    def _update_host(self):
        ...
        for pub in self.PUBLISH_PAGES:
            try:
                # Step 1: 获取Cookie验证页
                r1 = self.fetch(pub + '/', headers=self._get_headers(), timeout=10, verify=False)
                cookie_match = re.search(r'document\.cookie\s*=\s*"([^"]+)"', r1.text)
                
                cookie_str_list = []
                if cookie_match:
                    # 解析Cookie并拼成字符串，后续通过Header传递
                    ...
                    for part in parts:
                        ...
                        if '=' in part and 'path' not in part and 'max-age' not in part:
                            ...
                            cookie_str_list.append(f'{key.strip()}={val.strip()}')
                ...
                if cookie_str_list:
                    ajax_headers['Cookie'] = '; '.join(cookie_str_list)
                
                r2 = self.fetch(ajax_url, headers=ajax_headers, timeout=10, verify=False)
                ...
                for url in urls:
                    ...
                    if not url.startswith('http'):
                        continue
                    try:
                        test = self.fetch(url + '/', headers=self._get_headers(), timeout=8, verify=False)
        ...
        for h in self.hosts:
            try:
                test = self.fetch(h + '/', headers=self._get_headers(), timeout=8, verify=False)
                if test.status_code == 200 and len(test.text) > 1000:
                    self.host = h
    ...
    def init(self, extend=''):
        self._log('正在初始化...')
        # 注入域名优先（vbox 通过 _vbox_effective_hosts 设置 self.host）
        injected_host = getattr(self, 'host', '')
        if injected_host:
            self._log(f'使用注入域名: {injected_host}')
            self.hosts = [injected_host] + [h for h in self.hosts if h != injected_host]
        else:
            # 尝试更新域名
